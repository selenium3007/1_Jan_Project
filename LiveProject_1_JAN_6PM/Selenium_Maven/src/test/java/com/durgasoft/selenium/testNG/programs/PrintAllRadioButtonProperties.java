...
public class PrintAllRadioButtonProperties {
	...
	public void f() throws Exception {
		...
		for (int k = 1; k < 3; k++) {
			List<WebElement> radio1 = table.findElements(By.name("group"+k));
			System.out.println("Available radio buttons in group2:" + radio1.size());
			for (int i = 0; i < radio1.size(); i++) {
				radio1.get(i).click();
				for (int j = 0; j < radio1.size(); j++) {
					System.out.println(
							radio1.get(j).getAttribute("value") 
